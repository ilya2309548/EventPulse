...
## Минимальная логика самовосстановления (без оркестратора)

Минимальный набор сервисов EventPulse, который поддерживает «слабую» саморегуляцию:
- Ingest — точка входа событий.
- Rule Engine — принимает решения (открыть инцидент, запросить действие).
- Action Runner — исполняет действия (скрипт/рестарт/команда).
- Incident Store API — источник истины (состояния инцидентов).

Простая схема самовосстановления:
- Liveness: эндпойнт `/health` — процесс жив (HTTP 200).
- Readiness: эндпойнт `/ready` — готов работать (БД доступна, подписка/связь с очередью активна, правила загружены). Пока readiness=false, сервис:
  - не потребляет события (не подписывается/не коммитит оффсеты);
  - не публикует исходящие (кладёт в локальный outbox до готовности).
- Heartbeat: каждые 10–30 секунд сервис сообщает «я жив» — например, событием `svc.heartbeat{svc="ingest"}` или метрикой `svc_up=1` на `/metrics`.
  - Пропал heartbeat дольше N секунд → Incident Store открывает инцидент outage(X).
  - Rule Engine по такому инциденту публикует `action.requested(restart X)`.
  - Action Runner выполняет рестарт (скрипт/команда), публикует `action.completed/failed`.

Мини‑практики для реализации:
- Автоперезапуск процессов: Docker `restart: always` (или systemd/supervisord).
- Эндпойнты `/health` и `/ready` у каждого сервиса.
- Heartbeat: общий топик `svc.heartbeat` или HTTP‑гейдж; Incident Store отслеживает TTL.
- Inbox/Outbox: идемпотентность обработки (ключи `alert_id/incident_id/action_id`), коммит оффсета после коммита БД.
- Ретенция сообщений у брокера (чтобы после рестарта «догнать»); фиксированные consumer groups.

Упрощённый порядок запуска:
1) Поднять БД и шину сообщений (Kafka/NATS).
2) Прогнать миграции.
3) Запустить Incident Store API (готовность: БД ок).
4) Запустить Action Runner (готовность: БД ок, доступ к исполнителю).
5) Запустить Rule Engine (готовность: БД ок, правила загружены, подписка активна).
6) Запустить Ingest (готовность: БД ок, связь с шиной ок).
7) Протестировать контур: тест‑событие → `incident.opened` → `action.requested` → `action.completed` → статус в Incident Store.

Если упал сервис (примеры):
- Action Runner: действия временно не исполняются, события накапливаются; супервизор перезапускает; после старта дочитывает backlog; heartbeat/инцидент/рестарт — контролируется EventPulse.
- Rule Engine: решения не рождаются, `alert.raised` копится в топике; после перезапуска дочитывает; inbox исключает дубли.
- Ingest: новые алерты временно не принимаются; супервизор перезапускает; outbox защищает уже принятые перед падением; для PoC достаточно ретраев Alertmanager.
- Incident Store: состояния временно не обновляются; I1/AC/AF копятся; после старта идемпотентно применяются.

Почему этого достаточно:
- Readiness предотвращает «ломаный» старт.
- Liveness + внешний супервизор дают автоперезапуск.
- Heartbeat + событийная компенсация обеспечивают реакцию на «тихую смерть» или зависание.
- Inbox/Outbox и ретенция сообщений позволяют безопасно продолжать после рестартов.