...
### Предварительно
- Два контейнера Action Runner: `runner-a` и `runner-b`.
- Оба подписаны на один топик `action.requested` в одной consumer group (например, `group=action-runner`).
- Оба отправляют heartbeat и имеют readiness/liveness; для контейнеров включён auto‑restart (`Docker restart=always`).

### Последовательность

Нормальная работа
- Оба раннера живы; брокер распределяет сообщения так, что каждое `action.requested` обрабатывается ровно одним раннером.
- Inbox/идемпотентность исключают повторы.

Падение `runner-a`
- Liveness у `runner-a` проваливается → Docker автоматически пытается перезапустить контейнер.
- Heartbeat от `runner-a` пропадает → Incident Store открывает инцидент `outage(action-runner-a)`.
- Rule Engine эмитит `action.requested(restart runner-a)`.

Перехват задач `runner-b`
- Consumer group делает ребаланс: `runner-b` становится единственным потребителем и продолжает выполнять весь backlog `action.requested`.
- Само действие рестарта тоже попадает к `runner-b`: он выполняет скрипт/команду перезапуска `runner-a` (или вызывает Docker API для `start/restart`), публикует `action.completed` (или `action.failed` при неуспехе).

Восстановление `runner-a`
- После успешного рестарта `runner-a` проходит readiness, возобновляет heartbeat.
- Incident Store переводит инцидент в `resolved` (или оставляет `failed`, если рестарт не помог).
- Consumer group снова ребалансится: оба раннера делят поток событий; дубликатов нет благодаря inbox и ключам.

### Мини‑практики (только для Action Runner)
- Docker `restart: always` для контейнеров раннеров.
- Эндпойнты `/health` (liveness) и `/ready` (readiness).
- Heartbeat каждые 10–30 секунд (`svc.heartbeat{svc="action-runner-a|b"}` или `svc_up=1` на `/metrics`).
- Inbox/Outbox и уникальные ключи (`action_id`) для идемпотентности; коммит оффсета только после коммита БД.
- Фиксированная consumer group `action-runner` и достаточная ретенция брокера для «догонки» после рестартов.

Остальные компоненты (Ingest, Rule Engine, Incident Store) остаются без дополнительной логики самовосстановления, кроме базового авто‑перезапуска контейнера.
